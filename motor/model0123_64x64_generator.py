...
from model0123_64x64 import create_motor_generator
import numpy as np
...
# モデルのパス
model_path = 'motor/output/model0123_64x64'
...
for folder_path in generator_paths:
    ...
    try:
        ...
        for label in [40, 80, 120, 160, 200, 240]:
            # 当該ラベルの繰り返し
            labels = tf.constant([label] * num_img_per_label, dtype=np.float32)
            labels = tf.reshape(labels, (num_img_per_label, 1))  # ラベルの形状を調整
            ...
            for i in range(num_img_per_label):
                ...
                save_path = os.path.join(folder_path, "generated_img_label_%d_%d.png" % (label, i))
                img.save(save_path)
