...
# 生成器の修正
# Total params: 5796481
generator = keras.Sequential(
    [
        layers.Input(shape=(LATENT_DIM,)),
        layers.Dense(8 * 8 * 512),  # この層のニューロン数を増やして特徴マップのサイズを増加
        layers.Reshape((8, 8, 512)),
        layers.Conv2D(256, kernel_size=3, padding="same"),
        layers.BatchNormalization(),
        layers.LeakyReLU(alpha=0.2),
        layers.UpSampling2D(),  # 16x16 サイズに拡大
        layers.Conv2D(128, kernel_size=3, padding="same"),
        layers.BatchNormalization(),
        layers.LeakyReLU(alpha=0.2),
        layers.UpSampling2D(),  # 32x32 サイズに拡大
        layers.Conv2D(64, kernel_size=3, padding="same"),
        layers.BatchNormalization(),
        layers.LeakyReLU(alpha=0.2),
        layers.UpSampling2D(),  # 64x64 サイズに拡大
        layers.Conv2D(32, kernel_size=3, padding="same"),
        layers.BatchNormalization(),
        layers.LeakyReLU(alpha=0.2),
        layers.UpSampling2D(),  # 128x128 サイズに拡大
...
# 生成器のアーキテクチャ図を生成
plot_model(generator, to_file='WGAN-gp/output/architecture/generator_model_v3.png', show_shapes=True, show_layer_names=True)

# 識別器のアーキテクチャ図を生成
plot_model(discriminator, to_file='WGAN-gp/output/architecture/discriminator_model_v3.png', show_shapes=True, show_layer_names=True)
...
class GANMonitor(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        for i in range(self.num_img):
            ...
            img.save("WGAN-gp/output/samplev3/generated_img_%03d_%d.png" % (epoch, i))
...
if __name__ == "__main__":
    ...
    output_dir = "WGAN-gp/output/samplev3"
    ...
    # load_dir = "WGAN-gp/output/samplev3"
    ...
    gan.fit(
        ...
        callbacks=[
            ...
            LossCSVLogger(filename="WGAN-gp/output/samplev3/loss_log.csv")
        ],
    )
