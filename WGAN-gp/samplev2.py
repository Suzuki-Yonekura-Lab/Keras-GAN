...
# 生成器の修正
# Total params: 4900449
generator = keras.Sequential(
    [
        keras.Input(shape=(LATENT_DIM,)),
        layers.Dense(8 * 8 * 256),  # 層のニューロン数を増やして特徴マップのサイズを増加
        layers.Reshape((8, 8, 256)),
        layers.Conv2DTranspose(128, kernel_size=4, strides=2, padding="same"),  # 16x16 サイズに拡大
        layers.BatchNormalization(),
        layers.LeakyReLU(alpha=0.2),
        layers.Conv2DTranspose(64, kernel_size=4, strides=2, padding="same"),  # 32x32 サイズに拡大
        layers.BatchNormalization(),
        layers.LeakyReLU(alpha=0.2),
        layers.Conv2DTranspose(32, kernel_size=4, strides=2, padding="same"),  # 64x64 サイズに拡大
        layers.BatchNormalization(),
        layers.LeakyReLU(alpha=0.2),
        layers.Conv2DTranspose(1, kernel_size=4, strides=2, padding="same", activation="tanh"),  # 128x128 サイズに拡大
    ],
    name="generator",
)
generator.summary()
...
class GANMonitor(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        for i in range(self.num_img):
            ...
            img.save("WGAN-gp/output/samplev2/generated_img_%03d_%d.png" % (epoch+50, i))
...
if __name__ == "__main__":
    ...
    output_dir = "WGAN-gp/output/samplev2"
    ...
    load_dir = "WGAN-gp/output/samplev2"
    ...
    gan.fit(
        ...
        callbacks=[
            ...
            LossCSVLogger(filename="WGAN-gp/output/samplev2/loss_log.csv")
        ],
    )
