...
def combine(num, radius, output_name):
    # 3つの画像のファイル名をリストに格納する
    image_files = ["cGAN_practice/output/" + output_name + "/generated_img_{:03d}_radius_{}_{}.png".format(num-1, radius, z) for z in range(3)]

    # 3つの画像を読み込む
    images = [Image.open(filename) for filename in image_files]

    # 1x3の新しい画像を作成する
    new_image = Image.new('RGB', (images[0].width * 3, images[0].height))

    # 3つの画像を1x3の配置に結合する
    for index, image in enumerate(images):
        new_image.paste(image, (image.width * index, 0))
    ...
    new_image.save(output_dir + "/combined_image_epoch" + str(num) + "_radius_" + str(radius) + ".png")


def combine_images(nums, radius, output_name):
    # 各画像のファイル名をリストに格納する
    image_files = [
        "utils/output/cgan/" + output_name + f"/combined_image_epoch{num-1}_radius_{radius}.png" for num in nums
    ]
...
radiuses = [20, 30, 40, 50, 60]
output_name = "samplev3"

for num in nums:
    for radius in radiuses:
        combine(num, radius, output_name)

# 関数を実行して結合された画像を生成する
for radius in radiuses:
    combine_images(nums, radius, output_name)
