...
def combine_images(nums, output_name):
    # 各画像のファイル名をリストに格納する
    image_files = [
        "utils/output/cgan/" + output_name + f"/combined_image{num}.png" for num in nums
    ]
    ...
    for j in range(3):
        ...
        draw.text((x_offset, y_offset + image.height), f"Label {nums[j]-1}", fill="black", font=font)
